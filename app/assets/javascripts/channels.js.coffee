...
$ ->
  ...
  window.perform_play = (message) ->
    ...
    for sound, file_name of sounds
      return play_sound(file_name) if message.trim().match(sound)
    if message.trim().match(/^http/) or message.trim().match(/^file/)
      play_sound_from_url(message.trim())


  # perform(command, message)
  perform = (command, message) ->
    window["perform_" + command](message.split(" ").slice(1).join(" "))
  ...
  # play_sound_from_url(file_name)
  play_sound_from_url = (url) ->
    console.log
    $('audio')[0].setAttribute('src', url)
    $('audio')[0].play();
  ...
  window.channel_update_interval = setInterval(update_channel, 3000)
